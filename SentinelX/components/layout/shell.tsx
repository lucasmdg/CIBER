...
export function Shell({ children, user }: { children: React.ReactNode; user?: { name?: string | null; role?: string } }) {
  ...
  const systemStatus = React.useMemo(() => {
    ...
    if (telemetry.cpu > 85 || telemetry.mem > 90) {
      return { label: "CRITICAL", color: "text-danger border-danger/30 bg-danger/5 animate-pulse" };
    }
    if (telemetry.cpu > 65 || telemetry.mem > 75) {
      return { label: "WARNING", color: "text-warning border-warning/30 bg-warning/5" };
    }
    return { label: "SECURE", color: "text-success border-success/30 bg-success/5" };
  }, [telemetry]);
    ...
    <ToastProvider>
      <div className="grid-bg min-h-screen">
        <header className="sticky top-0 z-40 border-b border-white/5 bg-ink-950/70 backdrop-blur">
          <div className="container flex h-14 items-center justify-between">
            ...
            <div className="flex items-center gap-3 text-xs text-slate-400">
              <span className="hidden sm:inline font-mono">SOC-1 · {new Date().toLocaleDateString()}</span>
              <span className={cn("rounded border px-2 py-0.5 font-mono text-[10px] font-bold uppercase", systemStatus.color)}>
                {systemStatus.label}
              </span>
              {user && <span className="hidden md:inline font-mono text-slate-300">{user.name} · {user.role}</span>}
        ...
        <div className="container grid grid-cols-1 gap-6 py-6 lg:grid-cols-[240px_1fr]">
          <aside className="glass sticky top-20 h-fit p-3 flex flex-col gap-6">
              ...
              {nav.map((n) => {
                ...
                return (
                  <Link
                    ...
                    href={n.href}
                    className={cn(
                      "group flex items-center gap-2.5 rounded-md px-3 py-2 text-xs font-semibold tracking-wider uppercase transition relative overflow-hidden",
                      active
                        ? "bg-cyber-500/10 text-cyber-300 border border-cyber-500/20 shadow-neon"
                        : "text-slate-400 hover:bg-white/5 hover:text-white border border-transparent"
                    )}
                  >
                    <Icon className="h-4 w-4 transition duration-300 group-hover:scale-110 group-hover:text-cyber-300" />
                    {n.label}
                  </Link>
                );
              })}
